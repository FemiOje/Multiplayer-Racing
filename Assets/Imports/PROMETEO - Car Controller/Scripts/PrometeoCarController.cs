...
public class PrometeoCarController : MonoBehaviour
{
    ...
    */
    ...
    private void OnEnable()
    {
        ...
        _carActionMap = _inputActionAsset.FindActionMap("Car");
        _throttleAction = _carActionMap.FindAction("Throttle");
        _steerAction = _carActionMap.FindAction("Steer");
        
        _carActionMap.Enable();
    }

    private void OnDisable()
    {
        _carActionMap.Disable();
    }
    ...
    void HandleThrottle()
    {
        _throttleInputValue = _throttleAction.ReadValue<float>();
        if (_throttleInputValue > 0)
        {
            CancelInvoke("DecelerateCar");
            deceleratingCar = false;
            GoForward();
        }
        else if (_throttleInputValue < 0)
        {
            CancelInvoke("DecelerateCar");
            deceleratingCar = false;
            GoReverse();
    ...
    {
        _steeringInputValue = _steerAction.ReadValue<float>();
        if (_steeringInputValue > 0)
        {
            TurnRight();
        }
        else if (_steeringInputValue < 0)
        {
            TurnLeft();
        }
        else
    ...
    private void StartHandbrakeSequence()
    {
        CancelInvoke("DecelerateCar");
        deceleratingCar = false;
        Handbrake();
    ...
    public void HandbrakePressed(InputAction.CallbackContext context)
    {
        if (context.started)
        {
            StartHandbrakeSequence();
